...
func (ts *TodoServer) registerTools() {
	...
	ts.mcpServer.AddTool(
		mcp.NewTool("todo_create",
			...
			mcp.WithString("type",
				mcp.Description("What kind of work? (feature=new capability, bug=fix issue, refactor=improve code, research=investigate, prd=product requirements doc, multi-phase=parent project, phase=project phase [requires parent_id], subtask=smaller task [requires parent_id])"),
	...
	ts.mcpServer.AddTool(
		mcp.NewTool("todo_update",
			mcp.WithDescription("Update a todo in two ways: (1) Add content to a specific section, or (2) Update metadata like status/priority. For content updates, both 'content' and 'section' are required. For metadata updates, use the 'metadata' object."),
			mcp.WithString("id",
				mcp.Required(),
				mcp.Description("Which todo to update (e.g., 'fix-login-bug')")),
			mcp.WithString("content",
				mcp.Description("Your notes, code, test results, or findings to add (requires 'section' to be specified)")),
			mcp.WithObject("metadata",
				mcp.Description("Update todo properties without adding content (alternative to content+section)"),
				mcp.Properties(map[string]any{
					"status": map[string]any{
						"type":        "string",
			...
			mcp.WithString("section",
				mcp.Description("Required when adding content. Where to add content (findings=research notes, tests=test results, checklist=task items, scratchpad=rough notes)")),
	...
	ts.mcpServer.AddTool(
		mcp.NewTool("todo_template",
			...
			mcp.WithString("priority",
				...
				mcp.DefaultString("high")),
			mcp.WithString("type",
				mcp.Description("Work type (feature, bug, refactor, research, prd, multi-phase, phase, subtask)"),
				mcp.DefaultString("feature")),
		),
